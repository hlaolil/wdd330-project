...
import { renderHeader, renderFooter, renderResourceCard, displayResources, displayFavourites, displayCollection } from './render.js';
import { addToFavourites, removeFromFavourites, addToCollection, removeFromCollection, markAsRead, markAsUnread } from './state.js';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('main-content').addEventListener('submit', (e) => {
    if (e.target.id === 'search-form') {
      ...
      const activeNav = document.querySelector('.nav-link.active')?.dataset.type || 'all';
  ...
  document.getElementById('main-content').addEventListener('change', (e) => {
    if (e.target.id === 'sort-select') {
      const query = document.getElementById('search-input').value.trim() || 'theology';
      const activeNav = document.querySelector('.nav-link.active')?.dataset.type || 'all';
