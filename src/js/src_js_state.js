import { displayResources, displayFavourites, displayCollection } from './render.js';
...
export function addToFavourites(resource) {
  ...
  if (!favourites.some(f => f.id === resource.id)) {
    ...
    displayFavourites();
    displayResources(JSON.parse(localStorage.getItem('lastResources') || '[]'), document.querySelector('.nav-link.active')?.dataset.type || null);
  }
}

export function removeFromFavourites(id) {
  let favourites = JSON.parse(localStorage.getItem('theologyFavourites') || '[]');
...
export function removeFromCollection(id) {
  let resources = JSON.parse(localStorage.getItem('theologyCollection') || '[]');
